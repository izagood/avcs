...
## 11. 정정 공지 — 이 재설계가 **의도 없이** 떨어뜨린 원칙 (2026-08-27)

이 재설계는 목표를 달성했다. 코어에서 **언어/AST 인식**을 제거한 것은 옳았고 §10에 실측으로
남아 있다. 그런데 같은 변경이 언어 중립성과 아무 관계 없는 원칙 하나를 함께 떨어뜨렸다:

> **`docs/00-overview.md` 원칙 2 — "파일 경로가 아니라 Entity ID가 정체성이다 → rename + edit가
> 자동 병합 가능"** (선행 사례 목록에서 **Pijul**("파일 이름/inode 정체성 분리 → rename·edit
> commute")에 명시적으로 귀속된 원칙).

이 누락은 **언어 중립화의 의도된 귀결이 아니었고, 트레이드오프로 기록되지도 않았다.** §9 검증
매트릭스(C1–C8)에도, §10.4 갭 표(H1–H6)에도 나타나지 않는다. 즉 알고 포기한 것이 아니라
설계 논의에서 사라진 것이다.

### 11.1 실재하는 회귀라는 증거

- `src/objects/types.ts`의 `OperationTarget.entityId`는 **"Stable entity id — for files, the
  path"**로 문서화되어 있다. 즉 정체성이 곧 경로다 — 원칙 2가 부정한 바로 그 모델.
- `keysOf(rename_file)`은 `[file:<fromPath>, file:<path>]`, 즉 **두 경로에만 키잉**한다. 파일의
  안정된 정체성이 없으므로 rename 전후를 같은 엔티티로 묶을 근거가 코어에 없다.
- 귀결: **한쪽이 파일을 옮기고 다른 쪽이 그 파일을 편집하는 동시 패턴이 자동 병합되지 않는다.**
  git이 못 하는 걸 avcs가 한다고 약속한 지점에서, 지금 코어는 git과 같은 수준이다.

비용은 agentic 사용에서 특히 크다. 병렬 에이전트 세션은 리팩터(=이동)와 편집을 동시에 만드는
것이 정상 작업 패턴이고, 그 조합이 매번 사람에게 올라간다.

### 11.2 결정 (오너 결정, 2026-08-27)

**`docs/00`이 창립 진리원(founding source of truth)이며, 이 누락은 정정해야 할 오설계다.**

- Entity-ID 정체성을 **복원한다.** 단, **언어 인식을 되살리지 않는다.**
- 정체성은 **경로 레이어**에 산다(파일이 무엇인지 추적하는 층). `merge3`는 계속 언어 무지
  — 텍스트만 본다. 두 원칙은 서로 충돌하지 않는다: 원칙 1(코어는 언어를 모른다)은 *머지 단위*에
  관한 것이고, 원칙 2(Entity ID가 정체성)는 *무엇을 머지하는가의 식별*에 관한 것이다. 파일의
  안정 식별자는 파일 내용의 문법을 알 필요가 없다.
- `docs/00`은 **수정하지 않는다.** 창립 문서로 그대로 둔다. 정정은 이 문서(§11)에 남는다.

### 11.3 후속

설계는 별도 명세 **[19 — 파일 정체성](19-entity-identity.md)** 이 담당한다. 이 문서는 그쪽을
가리키기만 한다 — §11의 역할은 "왜 §10에 이 항목이 없었는가"를 정직하게 기록하는 것이다.

그 명세가 밝힌 사실 하나를 여기에도 남긴다: 실패는 단순 미구현이 아니라 **순서 우연 의존**이다.
`applyOp`의 canonical 순서(`lamport, oid`)에서 `edit_file(P)` → `rename_file(P→Q)`는 정답을
만들지만, 역순은 `edit_file`이 `tree.get(P)`를 못 찾아 `current = opBase`로 **P를 되살린다** —
내용이 두 경로에 존재하고 편집은 죽은 경로에 남는다. 결정론은 지켜지지만 정합성이 절반만 맞는다.
