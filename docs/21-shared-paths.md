...
> **상태: 구현됨.** [16 — Workspace 스코프](16-workspace-scope.md) §7이 개념만 제시하고 코드는
> 한 줄도 없던 부품이다. 이것이 없으면 native workspace가 실제 프로젝트에서 git worktree를
> **대체할 수 없고**, 따라서 avcs의 단독성 원칙([16](16-workspace-scope.md) §2-1)이 실사용에서
> 성립하지 않는다. 구현이 스펙과 갈린 지점은 §8에 적었다.
...
이 절은 구현이 §3~§5와 다르게 결론 낸 곳만 적는다. 스펙이 A1/A2 이전에 쓰였으므로 코드가
기준이다.

1. **키가 바뀌면 symlink를 재지정한다.** §3.4의 의사코드는 "이미 `cache`를 가리키는 symlink →
   no-op"과 "존재하고 symlink 아님 → 경고"만 다루고, **다른 캐시를 가리키는 symlink**를 다루지
   않는다. 그대로 두면 lockfile을 바꾼 workspace가 낡은 환경에 영구히 묶여 S4가 실사용에서
   무의미해진다. 그래서 대상이 **이 스토어의 캐시 트리 안**을 가리키는 symlink이면(코어가
   스스로 만든 것이므로) 새 캐시로 재지정하고, 그 밖을 가리키면 사용자 것이므로 손대지 않는다.

2. **`mode: "copy"`에서 이미 있는 디렉터리는 경고가 아니라 정상 상태다.** §3.4를 글자대로 읽으면
   copy 모드의 두 번째 투영마다 S6의 "존재하고 symlink 아님" 경고가 뜬다 — copy 모드에서 실제
   디렉터리는 **기대되는 정상 상태**이므로 그것은 오경보다. 그래서 copy 모드에서 대상이
   디렉터리이면 `linked: true`, 경고 없음으로 둔다. S6는 기본값인 symlink 모드로 검증한다.

3. **copy 모드의 캐시는 사람이 채운다.** symlink 모드에서는 workspace 안에서 설치하면 그 바이트가
   캐시로 들어가므로 두 번째 workspace가 곧 이득을 본다. copy 모드에서는 설치가 **복사본**에
   들어가고 캐시는 비어 있는 채다. 코어가 되돌려 복사할 수는 없다 — 설치가 끝난 시점을 알 수
   없고, 아는 순간 원칙 2가 깨진다. 그래서 copy 모드에서 캐시를 채우려면 `linkSharedPaths`가
   보고하는 `cache` 경로에 직접 설치하고 재투영해야 한다. 키 변경 후 재구성도 사람이 그 복사본
   디렉터리를 지우는 일이다 — 코어가 캐시를 갱신하려고 사용자 데이터를 지우지는 않는다.

4. **`checkoutInto`는 시그니처를 유지한다.** §4는 `checkoutInto`가 공유 상태를 보고하기를
   기대하지만 반환형이 `string[]`이고 기존 호출자가 여럿이다. 그래서 `projectInto`가
   `{written, shared, skipped}`를 주고 `checkoutInto`는 그 `written`으로 위임한다 — 추가 전용이고,
   `git-sync`의 재투영도 같은 경로를 지나므로 링크가 계속 유지된다.

5. **symlink 미진입은 검증했다.** §3.5의 주장(`Dirent`의 타입 판정이 lstat 의미)을 별도
   스크립트로 재현했다: 디렉터리를 가리키는 symlink는 `isDirectory()` false, `isFile()` false,
   `isSymbolicLink()` true. 그래서 S8은 ignore 합성을 넣기 **전에도** 통과했고 실제로 실패한
   것은 S7·S8b다 — "copy가 위험한 쪽"이라는 §3.5의 진단이 맞았다. S8은 설정을 지운 뒤에도 op이
   0개임을 단정해서 그 의존을 lstat 하나로 좁혀 고정한다.

6. **공유 경로를 나중에 선언하면 그 안의 옛 op은 삭제로 캡처된다.** 히스토리에 이미 `node_modules`가
   있는 저장소에서 공유 경로를 선언하면, 다음 캡처가 그 경로들을 (디스크에 없는 것으로 보아)
   `delete_file`로 기록한다. `.avcsignore`에 항목을 추가할 때와 **완전히 같은** 기존 동작이며,
   실제로 원하는 정리이기도 하다. 투영 쪽은 §3.5의 대칭 방어가 그런 옛 항목을 쓰지 않고
   `skipped`로 보고하므로 라이브 캐시로 새지 않는다.

7. **`gc --shared`의 스코프 집합은 후하게 잡았다.** "유도되는 키"를 base view · 모든 line · 그
   각각 × 알려진 모든 workspace에서 계산한다. 키를 몇 번 더 유도하는 비용은 reduce 한 번이고,
   유도하지 못한 비용은 재설치다. 과다 근사가 값싼 실수 쪽이다.
