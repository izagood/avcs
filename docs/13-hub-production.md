...
| **E5** | `GET /have` serializes every oid every sync → O(total history) per sync, no cursor (hubServer.ts:153-158) | **since-cursor incremental sync** built on the existing op-log (A5) | Med (scale) | — |
