...
| **E5** | `GET /have` serializes every oid every sync → O(total history) per sync, no cursor (hubServer.ts:153-158) | **since-cursor incremental sync**: a general append-only `objlog` + `GET /sync?since=N` + a persisted per-hub pull cursor (full `/have` fallback). Incremental **pull** done; incremental **push** (idempotent re-POST of the local objlog delta) is a documented follow-up. **#99 (v5)**: the remaining cost was the request COUNT, not the oid list — `POST /objects/batch` / `POST /objects/fetch` move the negotiated delta in byte-bounded chunks with per-oid verdicts, so a per-request rate limit is no longer a throughput ceiling (`batch: true` on `GET /version`; per-object endpoints unchanged for older hubs) | Med (scale) | — |
