...
핵심 통찰: AVCS에서 코드 트리는 저장되는 것이 아니라 operation DAG를 `reduce()`해서 만든 **파생물**이다. 그래서 `checkout`은 "이 파일들을 쓴다"가 아니라 **"워킹트리를 이 뷰로 만든다"**여야 한다 — 뷰에 없는 파일은 지운다. 무엇을 지울지는 **지난 투영 기록**(`.avcs/projection.json`: 경로 → blob oid)으로 판단하고, 그 기록에 없는 것은 손대지 않는다.

git의 인덱스와 같은 역할이고 규칙도 같다:

| 파일 | 동작 |
|---|---|
| 지난 투영에 있고 대상 뷰에 없음 | **삭제** |
| 투영한 적 없음 (`node_modules`, ignore 대상, 사용자가 새로 만든 파일) | 보존 |
| 지난 투영에 있으나 **바이트가 달라짐** | 보존 + 보고 |

세 번째가 중요하다. 아직 캡처되지 않은 편집은 **작업물**이므로, 투영을 정확하게 만들려고 그것을 지우면 낡은 파일 하나를 없애는 대가로 작업을 잃는다. 그래서 blob oid까지 기록한다 — 없으면 "내가 쓴 그대로"와 "사용자가 고쳤다"를 구분할 수 없다.

`materialize --out <dir>` / `workspace project` 는 **내보내기**라 이 규칙에서 빠진다: 남의 디렉터리를 지우지 않고, 제자리 투영 기록도 건드리지 않는다.

그 파생물은 AVCS가 언제든 결정론적으로 재생성할 수 있으므로, git이 추적할 "실제 코드"는 AVCS가 언제든 결정론적으로 재생성할 수 있다. git은 깨끗한 projection을, AVCS(`.avcs/`)는 풍부한 의도·결정 히스토리를 담당한다.
