...
> **상태: 설계 합의 (구현 전).** 이 문서는 병렬 에이전트에게 남아 있는 마지막 git식 고통 — "head가 움직였으니 pull 하고 다시 하세요" — 을 제거하는 Phase 13–15의 설계 결론을 기록한다. 구현 순서와 사이징은 [07 — 로드맵](07-roadmap.md) 참조. 에이전트 표면(MCP)은 [18 — MCP 일급 커넥션](18-mcp-first-class.md)이 담당한다.
