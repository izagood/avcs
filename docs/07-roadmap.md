...
- **Phase 16 — MCP 일급화 (M1–M5)** 🟡: **M1 ✅** 토큰 기반 — 컴팩트 직렬화 기본 + 범용 `verbose`, 실패는 `{error,hint,nextActions}` 봉투(`RECOVERY` 표, 참조 도구는 테스트가 실재성 강제), 유계 읽기(`history` limit/cursor · `intent.list` limit · `materialize` filesLimit/pathsOnlyUnder + filesTotal/filesTruncated · `object.show` lines/maxBytes + bytes/truncated · `diff format:patch` = merge3 LCS 코어 위의 `unifiedDiff`), 신규 `avcs.guide`(정본 루프·규칙·도구 색인·에러맵을 **live 테이블에서 생성** → 드리프트 불가) + description 25단어 상한 — **27→28개 도구인데 description 873→489단어(-44%)** · M2 ⬜ **`avcs.sync.land`**(push→reduce→checkpoint→통합 자동, 결과는 landed | conflict 패킷 둘뿐, L) · M3 ⬜ **`avcs.context.build`** ContextPack(결정적 토큰 예산 절단, M/L) · M4 ⬜ resources/prompts/알림(`head-advanced`·`foreign-op-hot-key`, M) · M5 ⬜ `--profile core` 13종 경량 광고 + docs/06 권위 레퍼런스화(S/M)
